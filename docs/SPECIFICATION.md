...
**Immutable releases**: The top-level `immutable_releases` section manages `enabled` (Boolean), which defaults to `false` for new projects. The section uses the endpoint response field name, not an invented repository PATCH field. Tailor runs this stage after repository settings and before Actions policy. An absent section or `enabled` key stays unmanaged, including during default merging and `--recut`. Explicit `true` and `false` values stay unchanged in config. Existing-project `fit` preserves the live value. If the read lacks access, `fit` warns and omits the section.

Tailor reads `GET /repos/{owner}/{repo}/immutable-releases`, which returns required Boolean fields `enabled` and `enforced_by_owner`. A `404` means disabled only after repository admin access and token Administration-read access are confirmed through repository and workflow permission reads. Otherwise, the value stays unknown. Access errors produce `would skip (insufficient scope)` in `baste` and `would skip (insufficient scope)` in `alter`. A disabled declaration with `enforced_by_owner: true` produces `would skip (enforced by owner)` or `would skip (enforced by owner)`, without a write.
...
`.tailor.yml` has nine top-level sections: `license`, `repository`, `immutable_releases`, `actions`, `code_scanning`, `code_quality`, `ruleset`, `labels`, and `swatches`. The `actions` section is a map of repository Actions policy settings. The `code_scanning` section is a map of CodeQL default setup settings, and the `code_quality` section is a map of GitHub Code Quality settings. The `ruleset` section is a map of settings for the branch ruleset named `Tailor`. `path` values use the full path relative to `swatches/`, including the file extension where one exists. Extensionless files (e.g. `justfile`) are referenced as-is. The `repository`, `immutable_releases`, `actions`, `code_scanning`, `code_quality`, `ruleset`, and `labels` sections can be absent in a hand-written config. Default merging adds missing Actions defaults before policy management.
