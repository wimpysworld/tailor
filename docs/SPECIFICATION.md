...
Go selection also controls Dependabot and the managed Go fragments. Every generated Dependabot file contains `github-actions` and `nix`. Explicit true adds `gomod`; explicit false omits it. A missing file with an absent Go declaration uses the Go-enabled variant for legacy compatibility.

Dependabot uses [whole-file ownership](design/dependabot-ownership.md). An owned file starts with the exact first line `# Managed by Tailor: .github/dependabot.yml`, ending with LF or CRLF. The marker grants consent to complete-file replacement. A byte-order mark, leading blank line, partial marker, wrong path, extra text, or missing newline does not grant ownership. Matching unmarked canonical content also does not grant ownership.

For `first-fit` and `always`, with or without `--recut`, Tailor creates a missing file, reconciles an owned file, and preserves an unmarked regular file byte-for-byte. Back up a custom file before adding the marker because reconciliation discards custom schedules, groups, registries, comments, commit-message settings, and other custom fields. A language declaration, active mode, or `--recut` never grants ownership.

An omitted Dependabot entry or `never` skips both allowed filenames without inspection. Active management rejects any entry at `.github/dependabot.yaml`; linked parents; final symlinks; directories; special files; read failures; and input or output above 1 MiB. Preview reports the plan and performs no filesystem write.

For an owned file, explicit true or false selects the current canonical body. With an absent declaration, Tailor retains a state recognised from a finite set of current and historical canonical bodies. Unknown, changed, or ambiguous owned content is a conflict. Recognition accepts an LF or CRLF marker newline independently from an exact LF or whole-body CRLF body. Rendering always writes the LF marker and current LF body.

Dependabot preflight runs after default merging and before repository context, authentication, wiki enablement, or other writes. Publication occurs in the swatch stage after the licence stage. Before publication, Tailor rechecks the project root, both names, parent identity, destination identity and type, marker state, and exact bytes. Missing-file publication does not replace a file that appears after preflight.

When this run creates a missing `.github` parent for Pages or wiki, Dependabot accepts only the same privately captured directory identity and never replaces a concurrent parent. Linux and macOS support no-replace parent publication. Other platforms return `errors.ErrUnsupported` when Dependabot publication needs the missing parent; no runtime acceptance is claimed there.

Replacement uses a sibling temporary file, file sync, close, atomic rename, and directory sync. The final check-to-rename race remains, and the wider run is not transactional. Close and post-publication errors are returned. Reports retain a confirmed Dependabot result when publication succeeded before a later error.
...
9. **Execution order**: after config parsing, `alter` removes retired entries in memory before authentication. It then normalises the security prerequisites (automated security fixes, secret scanning push protection, and secret scanning non-provider patterns) and emits their warnings before validation. Next, it completes Dependabot preflight, required Go discovery, Pages preparation, and the managed-file preflight before token verification with `GET /user`. It completes Pages and local wiki safety checks and renders any required Go builder workflow before wiki enablement. For a declared public wiki, it enables `has_wiki` through the API if needed, then checks remote readiness and local adoption. A blocker stops the command before other writes. After readiness passes, it writes the changed config once and removes present retired workflow files. The same `GET /user` response resolves `{{GITHUB_USERNAME}}`, so verification adds no extra API call. It then applies repository settings, immutable releases, Actions policy, code scanning, Code Quality, the ruleset, labels, variables, Pages, wiki files, and the licence. In the swatch stage, it publishes Dependabot first, then managed development files and ordinary swatches. Pages files follow. `baste` uses `DryRun`. It reports wiki readiness blockers and continues the full preview, but writes and removes nothing. `alter` uses `Apply`, and `alter --recut` uses `Recut`.
