...
func TestManagedPlaywrightNixFixture(t *testing.T) {
	...
	check := exec.CommandContext(t.Context(), nix, "flake", "check", "--all-systems", "--no-build", "--no-write-lock-file", "path:"+filepath.ToSlash(root)) // #nosec G204 -- The executable and isolated fixture are controlled by the test.
	...
	defer cancel()
	expression := `let
  fixture = builtins.getFlake "path:` + filepath.ToSlash(root) + `";
