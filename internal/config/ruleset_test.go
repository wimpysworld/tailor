...
func TestValidateRuleset(t *testing.T) {
	tests := []struct {
		...
		{name: "actor invalid type", section: &model.RulesetSettings{BypassActors: &[]model.RulesetBypassActor{actor("OrganizationAdmin", new(1), "always")}}, wantErr: `invalid ruleset.bypass_actors[0].actor_type "OrganizationAdmin"; must be "RepositoryRole", "Team", "User", "Integration", or "DeployKey"`},
		...
		{name: "actor invalid mode", section: &model.RulesetSettings{BypassActors: &[]model.RulesetBypassActor{actor("Team", new(7), "never")}}, wantErr: `invalid ruleset.bypass_actors[0].bypass_mode "never"; must be "always", "pull_request", or "exempt"`},
