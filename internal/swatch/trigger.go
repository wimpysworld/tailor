...
// TriggerCondition maps a swatch path to a config field and the value that
// must match for the swatch to be deployed.
...
func LookupTrigger(path string) (TriggerCondition, bool) {
	tc, ok := triggerConditions[path]
	return tc, ok
}

// EvaluateTrigger returns true when the path has no trigger condition or when
// the matching field on repo satisfies the condition. It returns false if repo
// is nil and a trigger exists, or if the field value does not match. The repo
// parameter must be a pointer to a struct with yaml-tagged fields.
func EvaluateTrigger(path string, repo any) bool {
	tc, ok := LookupTrigger(path)
	if !ok {
		return true
	}
